...
KlvStream = function() {
  ...
  this.push = function(packet) {
    if(buffer.length > 0) {
      ...
      // Skip over the M2TS packet header and the adaptation field, if it is present, to get to the rest of the KLV payload
      let offset = M2TS_PACKET_HEADER_SIZE + 1;
      if (((packet[3] & 0x30) >>> 4) > 1) {
        let adaptationLength = packet[4];
        offset += adaptationLength;
      }

      updatedBuffer.set(packet.subarray(offset, offset + klvPacketSize - buffer.length), buffer.length);
