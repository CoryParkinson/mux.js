...
/**
 * This method parses KLV packets from MPEG-TS packets. It assumes each KLV packet will be contained in exactly
 * two TS packets.
 */
KlvStream = function() {
  var klvPacketSize = 0,
  ...
  this.push = function(packet) {
    if(buffer.length > 0) {
      let updatedBuffer = new Uint8Array(klvPacketSize);
      updatedBuffer.set(buffer);
      // TODO: This almost certainly will not work with real data sources. The magic number 52 is the number
      //  of bytes to skip over, presumably from the MPEG-TS packet header. This is likely to be different
      //  for each data source since MPEG-TS encoding differs slightly between data sources. We should add
      //  some MPEG-TS parsing here to skip over the bytes we don't care about.
      updatedBuffer.set(packet.subarray(52, 52 + klvPacketSize - buffer.length), buffer.length);
      this.trigger('data', { klv: updatedBuffer, pts: pts });
      ...
      if(index >= 0) {
        ...
        const berLengthFirstByte = buffer[16];
        const lengthBytes = berLengthFirstByte & 0x7F;
        // KLV length does not include the UAS key or the length field itself
        klvPacketSize = lengthBytes === 1 ? buffer[17] + 17:
          (buffer[17] << 8) + buffer[18] + 19;
      }
    }
  };
